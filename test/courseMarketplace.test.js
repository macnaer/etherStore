...
contract("CourseMarketplace", (accounts) => {
  ...
  describe("Order the new course", () => {
    before(async () => {
      await _contract.orderCourse(courseId, proof, {
        from: buyer,
        value,
      });
    });

    it("should NOT allow to reOrder already owned course", async () => {
      await catchRevert(
        _contract.orderCourse(courseId, proof, {
          from: buyer,
          value,
        })
      );
    ...
    it("can get the Orderd course hash by index", async () => {
      const index = 0;
      ...
      assert.equal(
        courseHash,
        expectedHash,
        "Course hash is not maching the hash of Orderd course!"
      );
    });

    it("should match the data of the course Orderd by buyer", async () => {
  ...
  describe("Activate the Orderd course", () => {
  ...
  describe("Deactivate course", () => {
    ...
    before(async () => {
      await _contract.orderCourse(courseId2, proof2, { from: buyer, value });
  ...
  describe("ReOrder course", () => {
    ...
    it("should NOT reOrder when the course doesn't exist", async () => {
      const notExistingHash =
        "0x5ceb3f8075c3dbb5d490c8d1e6c950302ed065e1a9031750ad2c6513069e3fc3";
      await catchRevert(
        _contract.reorderCourse(notExistingHash, { from: buyer })
      );
    });

    it("should NOT reOrder with NOT course owner", async () => {
      const notOwnerAddress = accounts[2];
      await catchRevert(
        _contract.reorderCourse(courseHash2, { from: notOwnerAddress })
      );
    });

    it("should be able reOrder with the original buyer", async () => {
      const beforeTxBuyerBalance = await getBalance(buyer);
      const beforeTxContractBalance = await getBalance(_contract.address);

      const result = await _contract.reorderCourse(courseHash2, {
        from: buyer,
        value,
      });
      ...
      assert.equal(
        course.state,
        exptectedState,
        "The course is not in Orderd state"
      );
      assert.equal(
        course.price,
        value,
    ...
    it("should NOT be able to reOrder Orderd course", async () => {
      await catchRevert(_contract.reorderCourse(courseHash2, { from: buyer }));
    });
  });
